...
const ProjectDetailPage = ({ params }: { params: { id: string } }) => {
  const projects: { [key: number]: ProjectDetail } = {
    1: {
      ...
      role: "設計・開発・連携調整",
      background: "ECナビ（ポイントサイト、MAU約45万人）とゴシップガールズトーク（女子専用トークルームサービス）は既に提携していたが、従来はAFFITというASPを利用したファイル連携式でポイント付与を行っていた。AFFIT経由だと、ユーザーがコメントしてからポイント付与まで数日かかり、ゴシガルがサービス利用履歴に載ってしまう問題があった。また、広告面での掲載では検索しないと案件が出てこないため、ユーザーにとって分かりづらかった。",
      challenges: [
        "AFFITを使わないリアルタイムポイント付与システムの構築",
        "データの量・保存期間を考慮したDB設計（毎月100万〜300万件の想定）",
        "AFFIT経由とAPIでのポイント付与が重複しないリリーススケジュールの調整",
      ...
      solutions: [
        ...
        "タスクアラートでの掲載により、リアルタイム付与案件として表示",
        "Datadogによるエラー検知とSlack通知システムの実装"
      ],
      results: [
        "CV数が約30倍に向上（8月51,083件 → 9月1,841,094件）",
        "売上が約30倍に増加（8月30,649円 → 9月1,104,656円）",
        "リアルタイムポイント付与により、ユーザー体験が大幅改善",
        "サービス利用履歴に載らない形でのポイント付与を実現",
    ...
    2: {
      ...
      teamSize: "2名",
      role: "設計・開発",
      background: "ECナビではCSをイー・ガーディアン株式会社（EG）に外注しており、ECナビとのやり取りをBacklogで行っている。従来の運用では、EGさんが担当者を指定してBacklogに起票しても通知はどこにも来ず、各課題担当者は担当になった課題があるかを確認しに行く必要があった。課題の起票ペースは日によって数回〜数日に1回と幅があり、対応状況や更新の通知が来なかったため、都度アクセスして確認するしか方法がなかった。",
      challenges: [
      ...
      results: [
        "課題の対応漏れがほぼ0になり、EGさんからの進捗伺いが来ない状況を実現",
    ...
    3: {
      ...
      teamSize: "約2名",
      role: "設計・開発・効果測定・保守",
      background: "ECナビでは過去何度か広告コードの書き換えによる不正CVが発生しポイント付与が起きてしまう事件が起きている。当時時点で、ECナビのASP繋ぎこみでは、ASP側にはユーザーIDしか渡っていないため、案件のすり替え等による成果の不正が行われる可能性があった。そのため多角的なアプローチを行い不正を阻止する方針で、ナビとASP間での不正対策案として考案した。",
      challenges: [
    ...
    4: {
      ...
      teamSize: "3名",
      role: "設計・開発・運用",
      background: "従来、友達紹介やポイント懸賞でのギフト券配布は手作業で行われており、GodsでAmazon券を発行→ダウンロード→CSVに貼り付け→メール配信という流れだった。この手作業はセキュリティ的にも運用的にも適切ではなく、誤操作で不要なギフト券を発券してしまう可能性や、悪意を持った運用者がギフトコードを利用する可能性があった。",
      challenges: [
        "外部API（Gods、Digico）との連携による複雑な処理フロー",
      ...
      technicalDetails: {
        ...
        database: "GIFT_GRANT_HISTORY（ギフト付与履歴）、GIFT_TYPE（ギフト金種）、GIFT_CATEGORY（ギフト券種）の3つのテーブルを設計。tradeIdによる追跡可能な発券システム。",
        apis: ["Gods API", "Digico API", "SES（メール配信）", "ギフト券発券・照会API"],
    ...
    5: {
      ...
      teamSize: "2名",
      role: "設計・開発・Apple交渉",
      background: "定期的に行っているアプリのパッケージアップデートを行う際に、Appleの審査で落ちてしまったのが戦いの始まりだった。新規で開発を行っていないアプリで審査が落ちた為、チームが解散し知見がある人がいない&開発全く何もわからないという状況で、審査で指摘されていることについて調査・対応をする運びとなった。",
      challenges: [
    ...
    7: {
      ...
      teamSize: "3名",
      role: "計画・設計・実装",
      background: "ECナビのデータベースがEUC-JPエンコーディングで運用されており、文字化けの問題や国際化対応の必要性からUTF-8への移行が必要となった。",
      challenges: [
        "大量のデータを安全に移行する必要がある",
        "サービス停止を最小限に抑える必要がある",
        "移行後のデータ整合性を保つ必要がある",
    ...
    8: {
      ...
      teamSize: "2名",
      role: "計画・設計・実装",
      background: "ECナビでは様々な媒体、代理店等に広告出稿の依頼をしており、出稿にあたってCV（会員登録完了）を広告出稿先に成果通知する必要があった。従来、ソケット連携はエンジニアが対応する必要があり、レビューを含めて工数2日程度かかっていた。広告出稿の繋ぎこみは頻度がまちまちで、来る時は緊急度合いが高い事が多く、差し込みで対応することが多かったため、コンテキストスイッチが多く、リソースが本当に無いときなど困る状況になっていた。",
      challenges: [
        "エンジニアが対応する必要があるため、運用さん側でスケジュールのコントロールがしづらい",
        "連携のたびにconfigに一行追加&Class1ファイル追加の作業が思考停止で行われている",
        "広告出稿が終了している連携先があっても消されることがなく、負債が溜まっている",
        "ビジネスサイドが処理の流れを分かっておらず、仕様書の確認作業が伝言ゲームになってしまう"
      ],
      solutions: [
        "既存のシステム・連携作業に課題を感じ、管理画面化することを提案",
      ...
      results: [
        "エンジニア工数: 1~2日 → 0日",
        "リリース待ち: 2~5営業日 → 0.1営業日",
        "連携のたびに追加されていた大量の冗長的なコードが消せるようになった",
        "使われていない提携先も気軽に非利用にできるようになった",
        "広告出稿中なのかどうかが可視化できるようになった"
      ],
      learnings: [
        "既存システムの改善提案方法",
        "ビジネスサイドとの協業手法",
        "管理画面設計のベストプラクティス",
        "移行設計の重要性（並行運用を考慮した設計）",
        "チーム力向上のための勉強会開催の効果"
      ],
      technicalDetails: {
        architecture: "既存のクラス呼び出しをする前段で、新規作成した管理画面からの設定があれば利用するようにし、なければ既存の仕組みを通るようにして、並行運用ができるように移行設計を実施。",
        database: "リレーショナルデータベースに沿って実装し、先方が返してほしいパラメータは可変である為、別テーブルに切り出した。データ分析、管理画面でJSONよりカラムにしたほうが扱いやすいと考えた。",
        apis: ["経路仕様登録画面", "ソケット連携時、管理画面側の設定があればそちらを使うようにする"],
        challenges: ["移行も考えて実装をした", "リファクタは別に切り出した", "DB周りの設計では、リレーショナルデータベースに沿って実装するようにした"]
      }
    }
  };
