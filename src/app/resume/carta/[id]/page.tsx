...
const ProjectDetailPage = ({ params }: { params: { id: string } }) => {
  const projects: { [key: number]: ProjectDetail } = {
    ...
    2: {
      ...
      teamSize: "1名",
      role: "設計・開発",
      background: "CS対応業務では外部パートナーチームとタスク管理ツールでやり取りをしており、従来の運用では担当者への自動通知がなく、課題を都度手動で確認する必要があった。確認頻度のばらつきにより対応漏れが発生しやすい状況だった。",
      challenges: [
        "既存ツールのデフォルト通知機能がそのまま利用できず、独自の通知システムの構築が必要だった",
    ...
    3: {
      ...
      teamSize: "1名",
      role: "設計・開発・効果測定・保守",
      background: "既存のASP連携では不正防止の仕組みが不十分であり、より堅牢なセキュリティ対策が必要な状況だった。多角的なアプローチを検討した結果、新たな不正対策システムを設計・構築することになった。",
      challenges: [
        "不正CVポイント付与対策の必要性から、既存の連携方式の設計を根本から見直した",
    ...
    4: {
      ...
      teamSize: "1名",
      role: "設計・開発・運用",
      background: "従来、ギフト券配布は手作業で行われていた。これは誤操作で不要なギフト券を発券してしまう可能性や、内部統制上のリスクがあり、改善が必要な状態だった。",
      challenges: [
      ...
      technicalDetails: {
        ...
        database: "ギフト付与履歴、ギフト金種、ギフト券種の3つのテーブルを設計。追跡可能な発券システム。",
    ...
    5: {
      ...
      category: "モバイル開発",
      description: "管理チーム解散後の、ネイティブアプリ改修。",
      skills: ["Swift", "AppTrackingTransparency", "AuthenticationServices", "Sign in with Apple JS", "Firebase", "iOS開発"],
      teamSize: "1名",
      role: "設計・開発・Apple交渉",
      background: "定期的なアプリのパッケージアップデートを行う際に、Appleの審査で落ちてしまった。開発当時の担当者が不在の状況で、審査で指摘された内容についてゼロから調査・対応することになった。",
      challenges: [
        "アプリ開発の知見が乏しい状態からのスタート",
        "開発当時の担当者が不在で、ゼロベースでのキャッチアップが必要",
    ...
    9: {
      ...
      background: "主要システムは10年近く前に作られたレガシーシステムで、都度パッチ当ての改修を重ねてきて、対応するエンジニアのリソースコストが膨れてきていた。該当システムの実装当時から時間が経ち、想定されていた目的やコンセプトから外れた利用のされ方や、調査やリカバリのしにくさなど散見され、インシデントの見える化をしたことによってより目立つようになってきた。",
      challenges: [
        "20年近く前に作られたレガシーシステムの複雑化",
        "手作業による高い運用負荷",
        "計測精度の課題による出稿先との調整コストの発生",
        "システムの設計が古く、保守運用時のリスクが高い",
        "新たなビジネス要件や分析ニーズへの対応困難",
        "仕様が複雑で、担当者の異動等により全体を把握できる人材が不在な状態"
      ],
      solutions: [
