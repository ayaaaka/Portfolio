...
const ProjectDetailPage = ({ params }: { params: { id: string } }) => {
  const projects: { [key: number]: ProjectDetail } = {
    ...
    8: {
      ...
      teamSize: "2名",
      role: "計画・設計・実装",
      background: "ECナビでは様々な媒体、代理店等に広告出稿の依頼をしており、出稿にあたってCV（会員登録完了）を広告出稿先に成果通知する必要があった。従来、連携はエンジニアが対応する必要があり、レビューを含めて工数2日程度かかっていた。広告出稿の繋ぎこみは頻度がまちまちで、来る時は緊急度合いが高い事が多く、差し込みで対応することが多かったため、コンテキストスイッチが多く、リソースが本当に無いときなど困る状況になっていた。",
      challenges: [
      ...
      solutions: [
        ...
        "連携の管理画面を作成し、エンジニアが対応する必要がないようにした",
        "連携手法の違いを説明し、ビジネスサイドにも理解を深めてもらった"
      ],
      results: [
        "エンジニア工数: 1~2日 → 0日",
      ...
      technicalDetails: {
        ...
        apis: ["経路仕様登録画面", "連携時、管理画面側の設定があればそちらを使うようにする"],
