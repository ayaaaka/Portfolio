...
export function HobbyCard({ title, icon, description }: HobbyCardProps) {
    return (
        <div className="group p-4 border rounded-xl text-center">
            <FontAwesomeIcon
                icon={icon}
                width={100}
                className="mx-auto text-[100px]"
            />
            <p className="text-center mt-2.5">{title}</p>
            <div className="relative mt-2 p-2 bg-gray-800 text-white rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                {description}
